...
<head>
    ...
    <title>Quản Lý Đặt Sân - FIFAFIELD</title>
...
<main class="flex-grow max-w-7xl mx-auto px-6 py-12 w-full space-y-12">
    ...
    <div class="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div class="space-y-2">
            <h1 class="text-4xl font-black text-gray-900 tracking-tight uppercase leading-none">
                QUẢN LÝ <span class="text-[#008751]">ĐẶT SÂN</span>
            </h1>
            <p class="text-gray-400 font-bold uppercase text-[10px] tracking-[0.3em]">Hệ thống điều phối nhân sự FIFAFIELD 2026</p>
        </div>
        
        <!-- Quick Filters (Simplified) -->
        <form method="get" action="${pageContext.request.contextPath}/staff/bookings" class="flex flex-wrap items-center gap-4">
            <div>
            ...
            <a href="${pageContext.request.contextPath}/staff/bookings" class="text-[10px] font-black text-gray-400 uppercase tracking-widest hover:text-[#008751]">Xóa</a>
        </form>
    </div>
    ...
    <div class="space-y-6">
        <div class="flex items-center gap-4">
            ...
            <h2 class="text-[10px] font-black text-gray-400 uppercase tracking-[0.3em]">Danh sách điều phối</h2>
        </div>

        <c:choose>
            <c:when test="${empty bookings}">
                <div class="py-20 text-center bg-white elite-card border-2 border-dashed border-gray-100 shadow-sm">
                    <p class="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Không tìm thấy đơn đặt nào</p>
                </div>
            </c:when>
            <c:otherwise>
                <div class="space-y-4 pb-8">
                    <c:forEach var="b" items="${bookings}">
                        <div class="bg-white rounded-[2rem] border border-gray-100 shadow-lg shadow-gray-200/30 hover:shadow-xl hover:shadow-[#008751]/10 transition-all group relative overflow-hidden p-6 md:p-8 flex flex-col lg:flex-row items-center gap-6 lg:gap-10">
                            ...
                            <div class="flex flex-col sm:flex-row items-center gap-4 w-full lg:w-auto shrink-0">
                                <form method="post" action="${pageContext.request.contextPath}/staff/bookings" class="flex items-center gap-2 bg-gray-50 p-2 rounded-2xl border border-gray-100 w-full sm:w-auto">
                                    <input type="hidden" name="bookingId" value="${b.bookingId}" />
                                    <select name="status" class="bg-transparent border-none text-[10px] font-black uppercase tracking-widest text-gray-700 focus:ring-0 cursor-pointer px-3">
                                    ...
                                    <button type="submit" class="bg-gray-900 text-white p-2 rounded-xl hover:bg-[#008751] transition-colors">Update</button>
                                </form>
