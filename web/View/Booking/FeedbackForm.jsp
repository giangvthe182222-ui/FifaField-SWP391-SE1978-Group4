        ...
        <form method="post" action="${pageContext.request.contextPath}/customer/feedback" class="space-y-6">
            ...
            <div class="space-y-2">
                ...
                <select name="rating" required class="w-full px-4 py-3 rounded-2xl border border-gray-200 bg-white font-semibold">
                    ...
                    <option value="5" <c:if test="${feedback != null && feedback.rating == 5}">selected</c:if>>5 sao - Rất hài lòng</option>
                    <option value="4" <c:if test="${feedback != null && feedback.rating == 4}">selected</c:if>>4 sao - Hài lòng</option>
                    <option value="3" <c:if test="${feedback != null && feedback.rating == 3}">selected</c:if>>3 sao - Bình thường</option>
                    <option value="2" <c:if test="${feedback != null && feedback.rating == 2}">selected</c:if>>2 sao - Chưa hài lòng</option>
                    <option value="1" <c:if test="${feedback != null && feedback.rating == 1}">selected</c:if>>1 sao - Rất tệ</option>
                </select>
            </div>
