    ...
    <head>
        ...
        <style>
            ...
            .input-focus:focus {
                border-color: #008751;
                outline: none;
                box-shadow: 0 0 0 4px rgba(0, 135, 81, 0.05);
            }
        </style>
    </head>
        ...
        <header class="h-16 bg-white border-b border-gray-200 flex items-center justify-between px-6 shrink-0 z-20">
            <button type="button"
                    onclick="if (window.history.length > 1) {
                            ...
                            window.location.href = '${pageContext.request.contextPath}${dashboardPath}';
                        }"
        ...
        <script>
            (function () {
                ...
                function updatePrice() {
                    ...
                    schedules.forEach(function (s) {
                        if (s.id === selectedScheduleId)
                            schedulePrice = parseNum(s.price);
                    });
                    ...
                    if (vSel)
                        discount = parseNum(vSel.options[vSel.selectedIndex].getAttribute('data-discount'));
                ...
                function buildScheduleBoard() {
                    ...
                    if (activeDateFilter) {
                        orderedDates = orderedDates.filter(function (d) {
                            return d === activeDateFilter;
                        });
                    }
                    ...
                    if (!orderedDates.length) {
                        ...
                        if (board)
                            board.innerHTML = '<div class="py-10 text-center w-full"><p class="text-[9px] font-black text-gray-300 uppercase tracking-widest">Không có lịch cho ngày đã chọn</p></div>';
                        return;
                    }
                    ...
                    orderedDates.forEach(function (d) {
                        ...
                        var list = days[d].slice().sort(function (a, b) {
                ...
                var vSel = document.getElementById('voucherId');
                if (vSel)
                    vSel.addEventListener('change', updatePrice);
                document.getElementById('bookingForm').addEventListener('input', function (e) {
                    if (e.target && e.target.classList.contains('equipment-qty'))
                        updatePrice();
                });
