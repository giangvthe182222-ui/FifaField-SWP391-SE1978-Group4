                                    ...
                                    <c:forEach var="b" items="${entry.value}">
                                        <div class="border-2 rounded-[1.5rem] p-4 transition-all border-gray-50 bg-white hover:border-[#008751] hover:shadow-md">
                                            ...
                                            <div class="mt-3 flex flex-wrap gap-2">
                                                <span class="px-2 py-1 rounded-full text-[8px] font-black uppercase tracking-widest ${b.paymentStatus == 'paid' ? 'bg-emerald-50 text-[#008751] border border-emerald-100' : b.paymentStatus == 'deposited' ? 'bg-yellow-50 text-yellow-700 border border-yellow-200' : b.paymentStatus == 'pending refund' ? 'bg-amber-50 text-amber-600 border border-amber-100' : b.paymentStatus == 'refunded' ? 'bg-rose-50 text-rose-600 border border-rose-100' : 'bg-gray-100 text-gray-600 border border-gray-200'}">Payment: ${empty b.paymentStatus ? 'pending' : b.paymentStatus}</span>
