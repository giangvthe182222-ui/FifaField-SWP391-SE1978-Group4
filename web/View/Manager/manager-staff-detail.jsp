...
<main class="max-w-5xl mx-auto px-6 py-10 w-full flex-grow space-y-8">
    <div class="flex items-center justify-between">
        ...
        <c:if test="${param.success == 'true'}">
            <span class="text-emerald-700 bg-emerald-50 border border-emerald-200 px-3 py-1 rounded-full text-xs font-bold">Cập nhật trạng thái thành công</span>
        </c:if>
    </div>
    ...
    <div class="bg-white rounded-2xl shadow border border-gray-100 p-6 md:p-8 space-y-8">
        <div class="flex flex-col md:flex-row md:items-start gap-6">
            ...
            <form action="${pageContext.request.contextPath}/manager/staff/update-status" method="post" class="min-w-[180px]">
                <input type="hidden" name="staffId" value="${staff.userId}" />
                <input type="hidden" name="returnUrl" value="${pageContext.request.contextPath}/manager/staff/detail?id=${staff.userId}" />
                <label class="block text-xs font-bold text-gray-500 mb-2 uppercase tracking-wider">Trạng thái</label>
                <select name="status" onchange="this.form.submit()"
                        class="w-full rounded-xl border border-gray-200 px-3 py-2 text-sm font-semibold">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div class="rounded-xl border border-gray-100 p-5 bg-gray-50">
                <p class="text-xs font-bold text-gray-500 uppercase tracking-wider mb-2">Thông tin liên hệ</p>
                <p class="text-sm"><span class="font-semibold">SĐT:</span> ${staff.phone}</p>
            ...
            <div class="rounded-xl border border-emerald-200 p-5 bg-emerald-50">
                <p class="text-xs font-bold text-emerald-700 uppercase tracking-wider mb-2">Dữ liệu tính lương</p>
                <p class="text-sm text-gray-700">Số ca đã làm trong tháng <span class="font-bold">${monthLabel}</span>:</p>
                <p class="text-4xl font-black text-[#008751] mt-2">${staff.workedShiftsThisMonth}</p>
                <p class="text-xs text-gray-500 mt-2">Dùng số ca này để đối soát lương theo tháng.</p>
            </div>
        </div>
    </div>
</main>
