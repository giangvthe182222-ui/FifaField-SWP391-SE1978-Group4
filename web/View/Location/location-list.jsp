    ...
    <div class="max-w-7xl mx-auto px-6 py-12">
        ...
        <div class="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-12">
            <div class="space-y-2">
                <div class="flex items-center gap-3">
                    ...
                    <span class="text-[10px] font-black text-[#008751] uppercase tracking-[0.4em]">Infrastructure</span>
                </div>
                <h1 class="text-5xl font-black text-slate-900 tracking-tighter uppercase italic leading-none">FIFA FIELD SYSTEM</h1>
                <p class="text-slate-400 font-bold uppercase text-xs tracking-[0.2em]">Quản lý danh sách cụm sân toàn hệ thống</p>
            </div>
            
            <%-- Nút dẫn tới Servlet thêm mới --%>
            <a href="${pageContext.request.contextPath}/locations/add" 
               class="fifa-gradient text-white px-10 py-6 rounded-[2rem] font-black flex items-center gap-4 transition-all shadow-2xl shadow-[#008751]/40 hover:scale-105 active:scale-95 group">
                <i data-lucide="plus-circle" class="w-6 h-6 stroke-[3] group-hover:rotate-90 transition-transform"></i>
                    ...
                    <div class="bg-[#f1f5f9] rounded-[3.5rem] p-10 flex flex-col h-full relative overflow-hidden">
                        ...
                        <div class="flex justify-between items-start mb-10 relative z-10">
                            ...
                            <div class="flex flex-col items-end gap-2">
                                ...
                                <span class="text-[10px] font-bold text-slate-400 uppercase tracking-tighter">ID: ${loc.locationId.toString().substring(0,8)}</span>
                            </div>
                        </div>

                        <div class="relative z-10">
                            <h3 class="text-3xl font-black text-slate-900 mb-3 tracking-tight group-hover:text-[#008751] transition-colors">${loc.locationName}</h3>
                            ...
                            <div class="grid grid-cols-2 gap-4 mb-10">
                                <div class="bg-white/80 backdrop-blur-sm p-6 rounded-[2rem] border border-white shadow-sm">
                                    <p class="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1">Khu vực</p>
                                    <p class="text-sm font-black text-slate-800 tracking-tight">HÀ NỘI</p>
                                </div>
                                <div class="bg-white/80 backdrop-blur-sm p-6 rounded-[2rem] border border-white shadow-sm">
                                    <p class="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1">Hotline</p>
                                    <p class="text-sm font-black text-slate-800 tracking-tight">0${loc.phoneNumber}</p>
                                </div>
                            </div>

                            <div class="mt-auto flex gap-4">
                                <a href="${pageContext.request.contextPath}/fields?location_id=${loc.locationId}" 
                                   class="flex-[3] bg-white border-2 border-slate-200 hover:border-[#008751] hover:text-[#008751] py-5 rounded-[1.8rem] font-black text-center text-xs uppercase tracking-[0.15em] transition-all shadow-sm">
                                    DANH SÁCH SÂN CHI TIẾT
                                </a>
                                <button class="flex-1 bg-slate-900 text-white rounded-[1.8rem] hover:bg-black transition-all flex items-center justify-center shadow-lg">
        ...
        <c:if test="${empty locations}">
            <div class="text-center py-32 bg-white rounded-[5rem] border-4 border-dashed border-slate-100 mt-10">
                ...
                <h2 class="text-2xl font-black text-slate-300 uppercase italic tracking-widest">Hệ thống chưa có dữ liệu</h2>
                <p class="text-slate-400 font-bold mt-2">Vui lòng khởi tạo cụm sân đầu tiên để bắt đầu</p>
            </div>
        </c:if>
    </div>
